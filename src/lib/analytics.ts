...
export function computeMetadataGroups(
  ...
  const otherSessions = rest.flatMap((g) => {
    // We don't have the original sessions per group anymore,
    // so aggregate from the group stats
    return [];
  });

  // Since we can't reconstruct sessions from groups, re-aggregate from stats
